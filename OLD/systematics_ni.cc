...
1) Consider a true t distribution (dist_t_true).

2) From 1) derive the true distribution of th_y vs th_x (dist_th_x_th_y_true).

3) Consider angular fluctuations/resolution in th_x and th_y, independently in L and R arms. For
convenience are parametrised by m (affecting the L-R mean) and d (contributing to the L-R difference):
  \de\th_L = m - d/2
  \de\th_R = m + d/2
Potentially non-gaussian distributions can be used for de_th_y_L and de_th_y_R. m_x is modelled as gaussian.
d_x is not used at all, thus no model is needed.

4) Consider systematic effects - systematic modifications of th_x and th_y - misalignments lead to
shifts, optics to scaling, ... This means that the reconstructed th_y vs th_x distribution is:
  dist_th_x_th_y_re = renorm * dist_th_x_th_y_true(th_x_modif, th_y_modif)
where the _modif angles reflect the systematic bias and "renorm" is re-normalisation factor. The
transformation takes place in "IntegOverMX".

5) Reconstructed th_y vs th_x distribution ("dist_th_x_th_y_sm_acc") is obtained from "dist_th_x_th_y_re"
by integrating over m_x, m_y, d_y.

6) Reconstructed t distribution (dist_t_re) is obtained from the th_y vs th_x distribution ("dist_th_x_th_y_sm_acc")
by integrating over phi.

7) By default, step 5) includes "divergence" correction and step 6) includes "phi" acceptance correction.

8) The relative dsigma/dt effect of a systematic bias is then evaluated as
  effect = dist_t_re[with bias] / dist_t_re[without bias]
*/
...
double dist_m_x(double m_x)
{
	...
	const double r = m_x / si;
	return exp(-r*r/2.) / sqrt(2. * M_PI) / si;
}
...
{
	const double p = env.p;
	const double t = (th_x*th_x + th_y*th_y) * p*p;
...
double IntegOverMX(double x, double *p, const void *)
{
	...
	if (useBiases)
	{
		const double eff_corr = eff_intercept_error  +  eff_slope_error * (fabs(th_y_p) - eff_slope_fix_point);
	
		const double de_p_norm_corr = 1. + de_rel_p;
	
		// transformation from reconstructed (primed) to original/true (non-primed) angles
		// Th' = M Th + De Th  ==>  Th = M^-1 (Th' - De The)
		// Mi = M^-1
		const double th_x_ps = th_x_p - sh_th_x;
		const double th_y_ps = th_y_p - sh_th_y;
	
		const double Mi_xx = 1. / sc_th_x, Mi_xy = - tilt_th_x_th_y;
		const double Mi_yx = 0., Mi_yy = 1. / sc_th_y;
