...
void RemovePartiallyFilledBinsThetaXY(TH2D *h)
{
	...
	{
		auto [th_y_min, th_y_max] =	anal.fc_G.GetThYRange(th_x);
	...
	*/
	...
	for (int ev_idx = 0; ev_idx < ch_in->GetEntries(); ++ev_idx)
	{
		...
		*/
		...
		// define safe flag
		auto [safe_th_y_min, safe_th_y_max] = anal.fc_G.GetThYRange(anal.th_x_default);
		safe_th_y_min += 10E-6;
		safe_th_y_max -= 10E-6;

		const bool safe = fabs(k.th_y) > safe_th_y_min && fabs(k.th_y) < safe_th_y_max && fabs(k.th_x) < anal.th_x_safe_max;
