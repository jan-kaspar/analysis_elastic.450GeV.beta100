...
\> see e.g.~\plot{time_dependences/rate_vs_time.pdf}
...
\> TODO: add list of bunches used in different fills

\section[beam-cond-emittance]{Beam emittances}

\> \plot{time_dependences/emittance.pdf}: beam emittance as measured by BSRTs

\section[beam-cond-rates]{Rates}

\> plots of rates vs.~time
\>> \plot{time_dependences/event_category_rates.pdf}: rates of different event categories, per RP
...
\>>> single arm: $\displaystyle {v_x^F x^N - v_x^N x^F\over v_x^F L_x^N - v_x^N L_x^F}$
\>>> double arm: average of single arm, $\th_x^* = (\th_x^{*,R} + \th_x^{*,L}) / 2$
\>> vertical plane
\>>> single pot: $\displaystyle \th_y^{*} = \mp {y\over L_y}$
\>>> single arm: $\displaystyle \th_y^{*} = (\th_y^{*,F} + \th_y^{*,N}) / 2$
...
\> \plot{hit_distributions/hit_distributions_sel_cmp.pdf}: hit distribution with and without elastic event selection
...
\> \plot{resolution/vtx_y_vs_time.pdf}: time dependence of reconstructed $x^*$ RMS (without subtracting the resolution)
