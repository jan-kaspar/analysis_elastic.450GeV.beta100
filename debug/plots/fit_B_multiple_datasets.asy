...
for (string dataset : datasets)
{
	...
	draw(RootGetObject(f, dataset + "_combined/h_dsdt"), "eb", black);
	
	for (int ri : ranges.keys)
	{
		draw(RootGetObject(f, dataset + "_combined/fit_range_" + ranges[ri]), r_pens[ri]+1pt);
	}

	limits((0, 100), (0.025, 300), Crop);

	AttachLegend(BuildLegend(dataset, S), N);
}
