...
string row_captions[] = { "sector 45", "sector 45", "sectors 45 $\oplus$ 56" };
...
void DrawRatio(RootObject num, RootObject den, pen p, string label="")
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		draw(swToHours * (x, r), p, mCi+2pt + p);
		draw(swToHours * ((x-x_unc, r)--(x+x_unc, r)), p);
		draw(swToHours * ((x, r-r_unc)--(x, r+r_unc)), p);
	}
