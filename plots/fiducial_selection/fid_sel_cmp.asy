...
for (int dsi : datasets.keys)
{
	...
	for (int dgni : diagonals.keys)
	{
		...
		{
			for (string flavour : new string[] { "l", "h" })
			{
				RootObject obj = RootGetObject(f, "fiducial cuts/fc_"+cuts[ci] + "_" + flavour);
				draw(scale(1e6, 1e6), obj, "l", StdPen(ci+1), flavour + ": "+ cut_labels[ci]);
